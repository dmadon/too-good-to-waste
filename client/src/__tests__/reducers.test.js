...
test('SET_MY_ID', () => {
    ...
    expect(newState.myId).toBe(1);
    expect(initialState.myId).toBe('');
});

test('SET_SELECTED_PARTNER', () => {
    let newState = reducer(initialState,{
        type: SET_SELECTED_PARTNER,
        _id:2
    });

    expect(newState.selectedPartner).toBe(2);
    expect(initialState.selectedPartner).toBe('');
});
